print("Which character are you from 'Avengers?'")
print()
print("Answer these questions and let's find out.")
print()
print("Please use Y or N for Yes and Nno.")

likeGreen = input("Do you like the color green?: ")
if likeGreen == "Y":
  print("Aha! You must be the Hulk!")

IronIsCool = input("Do you think building robots is cool?: ")  
if IronIsCool == "Y":
  print("Aha! You must be Iron Man. Cool suit!")

TimeTravel = input("Do you like traveling through time?: ")  
if TimeTravel == "Y":
  print("Aha! You must be Captain America! Hi!")

Strong = input("Are you super strong?: ")
if Strong == "Y":
  print("WOW! You have got to be Thor!")
else:
  print("How sad...I guess you are not like anyone from 'Avengers.'")
